...
export default function MapSection() {
  ...
  return (
    <section 
      ref={sectionRef}
      className="bg-background py-32 lg:py-48 overflow-hidden relative transition-colors duration-500" 
      id="map-section"
    >
      ...
      <div className="mx-auto max-w-[1248px] px-5 relative z-10">
        <div className="text-center mb-24 max-w-[800px] mx-auto">
          <span className="map-text text-brand-lime font-bold tracking-[0.3em] uppercase text-xs mb-6 block">Global Presence</span>
          <h3 className="map-text text-5xl lg:text-7xl font-bold font-serif text-foreground mb-8 tracking-tight">
            Strategic <br />
            <span className="text-brand-blue">Global Operations</span>
          </h3>
          <p className="map-text text-xl text-muted-foreground leading-relaxed font-normal">
          ...
          {trackers.map((tracker) => (
            ...
            >
              ...
              <div className="relative flex items-center justify-center">
                 ...
                 <div className={cn(
                    "absolute w-8 h-8 rounded-full border border-current animate-ping opacity-20",
                    activeTracker === tracker.id ? "text-brand-lime" : "text-brand-blue"
                 )}></div>
                 <div className={cn(
                    "absolute w-12 h-12 rounded-full border border-current animate-pulse opacity-10",
                    activeTracker === tracker.id ? "text-brand-lime" : "text-brand-blue"
                 )}></div>
              </div>
              ...
              {/* High-tech Tooltip */}
              <div className={cn(
                "absolute bottom-full left-1/2 -translate-x-1/2 mb-6 transition-all duration-500",
                activeTracker === tracker.id ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-4 scale-90 pointer-events-none"
              )}>
                <div className="bg-card/95 backdrop-blur-xl border border-border shadow-[0_20px_50px_rgba(0,0,0,0.1)] p-5 rounded-2xl min-w-[200px]">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs font-bold text-brand-lime tracking-widest uppercase">{tracker.region}</span>
                    <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                  </div>
                  <h6 className="text-foreground text-lg font-bold mb-1">{tracker.label}</h6>
                  <p className="text-[10px] text-muted-foreground uppercase tracking-widest border-t border-border/50 pt-2 mt-2">Operational Status: Optimal</p>
                  
                  {/* Decorative corner accent */}
                  <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-card rotate-45 border-r border-b border-border"></div>
                </div>
              </div>

              {/* Label below dot (always visible but subtle) */}
              <div className={cn(
                "absolute top-full left-1/2 -translate-x-1/2 mt-3 transition-opacity duration-300",
                activeTracker === tracker.id ? "opacity-0" : "opacity-60 group-hover/dot:opacity-100"
              )}>
                 <span className="text-[10px] text-foreground font-bold uppercase tracking-tighter">{tracker.label}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Global Stats Footer */}
        <div className="mt-32 grid grid-cols-2 lg:grid-cols-4 gap-8 border-t border-border/50 pt-16">
           {[
             ...
             <div key={i} className="text-center group">
               <div className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-2 group-hover:text-brand-blue transition-colors">{stat.label}</div>
               <div className="text-3xl font-serif font-bold text-foreground tracking-tighter">{stat.value}</div>
             </div>
           ))}
        </div>
      </div>
